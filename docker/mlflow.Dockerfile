...
#
# Se arranca con --serve-artifacts: con esa opcion el servidor actua de proxy
# de artefactos y entrega a los clientes URIs portables del tipo
# mlflow-artifacts:/..., en lugar de rutas absolutas del sistema de ficheros.
# Ese es justo el problema que hacia inservible el historial local, cuyas
# rutas apuntan a C:/Users/... y no existen dentro de un contenedor Linux.
...
# mlflow completo (no mlflow-skinny): el servidor necesita sqlalchemy y alembic
...
# Se enumeran los hosts legitimos en lugar de abrir con "*".
CMD ["mlflow", "server", \
     "--host", "0.0.0.0", \
     "--port", "5000", \
